...
def generate_composits(prefix, i_file, c_file, ig_file, mg_file):
    ...
    if (s < 0.1 * mask.shape[0] * mask.shape[1]):
        ...
        if is_day:
            rsi.save_dataset('I01', writer='geotiff', enhance=False, filename=os.path.join(prefix, 'i01_ref.tiff'))
            rsi.save_dataset('I02', writer='geotiff', enhance=False, filename=os.path.join(prefix, 'i02_ref.tiff'))
            rsi.save_dataset('I03', writer='geotiff', enhance=False, filename=os.path.join(prefix, 'i03_ref.tiff'))
        rsi.save_dataset('I04', writer='geotiff', enhance=False, filename=os.path.join(prefix, 'i04_bt.tiff'))
        rsi.save_dataset('I05', writer='geotiff', enhance=False, filename=os.path.join(prefix, 'i05_bt.tiff'))
        ...
        rs.save_dataset('solar_zenith_angle', writer='geotiff', enhance=False, filename=os.path.join(prefix, 'mvcm.tiff'))
        del rs
        del s
        gc.collect()
...
def my_assim(path):
    ...
    for c_file in c_files:
        ...
        if (ig_file == "" or mg_file == "" or i_file == ""):
            ...
            generate_composits(
                os.path.join(path, ts), i_file, c_file, ig_file, mg_file
            )
        k += 1
...
sd = datetime(2012, 3, 1)
ed = datetime(2012, 11, 7)


t1 = time.time()
assimilate(
    names, sd, ed, './2012',
    BY_BOX, 1, assim_callback=my_assim
)
t2 = time.time()
